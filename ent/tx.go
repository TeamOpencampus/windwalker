...
func (tx *Tx) init() {
	tx.CollegeProfile = NewCollegeProfileClient(tx.config)
	tx.Company = NewCompanyClient(tx.config)
	tx.JobPost = NewJobPostClient(tx.config)
	tx.StudentAcademicProfile = NewStudentAcademicProfileClient(tx.config)
	tx.StudentProfile = NewStudentProfileClient(tx.config)
	tx.StudentWorkProfile = NewStudentWorkProfileClient(tx.config)
...
// applies a query, for example: CollegeProfile.QueryXXX(), the query will be executed
