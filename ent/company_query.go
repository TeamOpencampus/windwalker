...
func (cq *CompanyQuery) QueryPosts() *JobPostQuery {
	query := &JobPostQuery{config: cq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CompanyQuery) FirstID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (cq *CompanyQuery) FirstIDX(ctx context.Context) xid.ID {
...
func (cq *CompanyQuery) OnlyID(ctx context.Context) (id xid.ID, err error) {
	var ids []xid.ID
...
func (cq *CompanyQuery) OnlyIDX(ctx context.Context) xid.ID {
...
func (cq *CompanyQuery) IDs(ctx context.Context) ([]xid.ID, error) {
	var ids []xid.ID
...
func (cq *CompanyQuery) IDsX(ctx context.Context) []xid.ID {
	ids, err := cq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (cq *CompanyQuery) WithPosts(opts ...func(*JobPostQuery)) *CompanyQuery {
	query := &JobPostQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CompanyQuery) sqlAll(ctx context.Context) ([]*Company, error) {
	...
	if query := cq.withUser; query != nil {
		...
		nodeids := make(map[xid.ID][]*Company)
		for i := range nodes {
			if nodes[i].user_companies == nil {
				continue
			}
			fk := *nodes[i].user_companies
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_companies" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := cq.withPosts; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Posts = []*JobPost{}
		}
		query.withFKs = true
		query.Where(predicate.JobPost(func(s *sql.Selector) {
			s.Where(sql.InValues(company.PostsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.company_posts
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "company_posts" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "company_posts" returned %v for node %v`, *fk, n.ID)
			}
